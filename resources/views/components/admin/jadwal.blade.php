                        ...
                        <tr>
                            ...
                            <td>
                                @if ($item->status == 'Dipesan')
                                    <p class="p-1 text-center text-white fw-semibold m-0"
                                        style="width: 100px; border-radius:25px; font-size:13px; background-color: #282828;">
                                        {{ $item->status }}
                                    </p>
                                @else
                                    <p class="p-1 text-center text-white fw-semibold m-0"
                                        style="width: 100px; border-radius:25px; font-size:13px; background-color: #002379;">
                                        {{ $item->status }}
                                    </p>
                                @endif
                            </td>
                            <td class="d-flex align-items-center gap-2">
                                ...
                                <form action="{{ route('admin.deletejadwal', $item->id) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn border border-0 fw-bold text-white"
